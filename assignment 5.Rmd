...
Assume the scale readings are Normally distributed with unknown mean (the mean is 10
g if the scale is accurate). The standard deviation for the readings is 0.0002 g.
...
a) Give H0.
b) Suppose that x??= -5 and n=10. Calculate the test statistic and its P-value.
c) Repeat the procedure with the same sample mean value, but with n=20, 30, 40 and
50.
...
se <- sd/sqrt(n1)
t1 <- abs(x-0)/se
p1 <- pnorm(t1, lower.tail = F)
print(c("t-stats=",t1, "p-value = ", p1))#Ans
...
print("For U20S = ", nU20S)
print("For MCF7 = ", nMCF7)